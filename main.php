...
<form action = "main.php" action= "get"> <?php #creates a form section, all data submitted wil be sent to the main php file, data are sent as url variables by the method get.?>
...
if (isset ($_GET["materials"]) && (isset ($_GET["pro"]))) { #code to be executed if data is recieved from the materials list and the pro list, otherwise it wont work.
    $materials = $_GET["materials"]; #creates a materials list from the form data in materials.
    $pro = $_GET["pro"]; #creates a pro list from the form data in pro.
...
<?php
if (isset($_GET["txt2"])){ #if data is entered from the textbox, it will execute the code below.
    $text = $_GET["txt2"]; #stores the text entered into the $text
